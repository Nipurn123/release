...
// Monkey patch to fix validator errors
process.on('uncaughtException', (err) => {
    if (err.message && err.message.includes('validator is not a constructor')) {
        logger_1.logger.error('Caught validator constructor error - continuing execution');
        logger_1.logger.error(err.stack || err.toString());
    } else {
        throw err;
    }
});
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if ((0, wrapper_1.isChild)(wrapper_1.wrapper)) {
            try {
                ...
                logger_1.logger.error("Error in child process:", error);
                // Don't exit on error to allow development to continue
                return;
            }
        }
...
entry().catch((error) => {
    ...
    // Don't exit on error in development mode
    if (process.env.VSCODE_DEV !== '1') {
        wrapper_1.wrapper.exit(error);
    } else {
        logger_1.logger.error('Development mode: continuing despite error');
    }
});
